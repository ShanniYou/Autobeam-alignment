...
class ValueIteration(MDPAlgorithm):
    ...
    def solve(self, mdp: MDP, epsilon=0.001):
        mdp.computeStates()
        #print('check states', mdp.states)
        def computeQ(mdp: MDP, V: Dict[Tuple, float], state: Tuple, action: Any) -> float:
            # Return Q(state, action) based on V(state).
            k = 0
            for newState, prob, reward in mdp.succAndProbReward(state, action):
                if newState in mdp.states:
                    #print(newState, prob, reward, V[newState])
                    k += prob * (reward + mdp.discount() * V[newState])
        ...
        while True:
            ...
            for state in mdp.states:
                ...
                for action in mdp.actions(state):
                    k.append(computeQ(mdp, V, state, action))
                newV[state] = max(np.array(k))
                #print(f"state {state} found value {newV[state]}", mdp.states)
            numIters += 1
        ...
        print(mdp.states)
        print(("ValueIteration: %d iterations" % numIters), 'V', V)
        self.pi = pi
        self.V = V
